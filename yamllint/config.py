...
"""Configuration handling for yamllint."""
import fileinput
...
class YamlLintConfig:
    ...
    def enabled_rules(self, filepath):
        return [
            yamllint.rules.get(rule_id)
            for rule_id, val in self.rules.items()
            if val is not False
            and (
                filepath is None
    ...
    if isinstance(conf, dict):
        ...
        for optkey in conf:
            ...
            if optkey not in options:
                raise YamlLintConfigError(
                    f'invalid config: unknown option "{optkey}" for rule "{rule.ID}"'
                )
